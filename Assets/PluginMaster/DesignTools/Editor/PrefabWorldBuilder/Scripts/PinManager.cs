...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static void SetPinValues(Quaternion additionRotation)
        {
            ...
            var centerToPivot = Vector3.Scale(prefab.transform.position - bounds.center, strokeItem.scaleMultiplier);
        ...
        private static void PinInfoText(UnityEditor.SceneView sceneView)
        {
            ...
            { _paintStroke[0].prefab.name,
              $"P: {p.x.ToString("F2")}, {p.y.ToString("F2")}, {p.z.ToString("F2")}",
              $"R: {r.x.ToString("F2")}, {r.y.ToString("F2")}, {r.z.ToString("F2")}",
              $"S: {s.x.ToString("F2")}, {s.y.ToString("F2")}, {s.z.ToString("F2")}"};
